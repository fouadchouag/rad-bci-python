...
class BandpowerExtPlugin(BasePlugin):
    help = {
        'summary': 'Extract band power features by delegating to an external bandpower script via subprocess.',
        'inputs': {
        ...
        'parameters': [
            {'name': 'bands', 'type': 'dict', 'default': '{"delta":[1,4],"theta":[4,8],"alpha":[8,13],"beta":[13,30],"gamma":[30,45]}',
             'desc': 'Band definitions passed to the external script (hardcoded in payload).'},
        ],
        'gotchas': [
        ...
        'usage': 'Connect a segment and sampling frequency. The node writes input JSON, runs the external script, and reads the output JSON to emit features.',
    }
