...
class ClassifierMetricsPlugin(BasePlugin):
  ...
  'inputs': {'dataset': 'dict — {X: np.ndarray[N,d], y: np.ndarray[N], y_names: [str,str]}',
             'config_in': 'dict (optional) — {"folds": int}',
             'metrics_conf': 'dict (optional) — merged with config_in'},
  'outputs': {'config_out': 'dict — {"folds": int} current fold count'},
  'parameters': [ { 'default': 5,
                    'desc': 'Number of CV folds (2..10)',
                    'name': 'folds',
                    'type': 'int'}],
  'summary': 'Evaluates a binary dataset {X, y, y_names} from an upstream EEGClassifier '
             'using StratifiedKFold cross-validation with a StandardScaler + LogisticRegression pipeline. '
             'Displays accuracy ± std, precision/recall, and confusion matrix.',
  'usage': 'Connect a dataset dict (X, y, y_names) from EEGClassifier. '
           'Click "Evaluate (CV)" in the UI to run cross-validation. '
           'Folds can be adjusted via UI spinner or config_in.'}
