...
class RawWindowSlicerPlugin(BasePlugin):
    help = help = { 'gotchas': ['Timer-based streaming; window timing depends on system timer resolution.',
  ...
  'inputs': {'raw': 'mne.io.Raw — continuous recording to slice',
             'run': 'bool — True=play, False=pause (optional)',
             'reset': 'bool — True resets read position to 0 (optional)'},
  'outputs': {'segment': '2D float array [ch x samples] — windowed EEG chunk (Volts)',
  ...
  'parameters': [ { 'default': 2.0,
                    'desc': 'Window duration',
                    'name': 'win_s',
                    'type': 'float',
                    'unit': 's'},
                  { 'default': 50,
                    'desc': 'Overlap percentage between windows',
                    'name': 'overlap',
                    'type': 'int',
                    'unit': '%'},
                  { 'default': True,
                    'desc': 'Loop back to start when reaching end',
                    'name': 'loop',
                    'type': 'bool'},
                  { 'default': True,
                    'desc': 'Honor external run/reset commands',
                    'name': 'honor_ext_run',
                    'type': 'bool'}],
  'summary': 'Slices a continuous MNE Raw into overlapping windows via a QTimer.',
  'usage': 'Connect an MNE Raw; outputs streaming segments with configurable window size and overlap.'}
