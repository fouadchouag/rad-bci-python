...
class BandPowerExtractor(BasePlugin):
    help = help = {
        'summary': 'Aggregate PSD into canonical frequency bands (delta/theta/alpha/beta/gamma) as absolute or relative power.',
        'usage': 'Connect PSD and freqs outputs from a PSDWelch node. Adjust band edges and options in the properties panel.',
        'inputs': {
        ...
        'parameters': [
            {'name': 'psd_is_db', 'type': 'bool', 'default': False, 'desc': 'Set True if PSD input is in dB; linear conversion is applied internally'},
            {'name': 'relative', 'type': 'bool', 'default': True, 'desc': 'Normalize band power by total power per channel'},
            {'name': 'bands', 'type': 'list[tuple]', 'default': [('delta',1.0,4.0),('theta',4.0,8.0),('alpha',8.0,13.0),('beta',13.0,30.0),('gamma',30.0,45.0)], 'desc': 'Frequency band definitions as (label, f_lo, f_hi)'},
        ],
        'gotchas': [
