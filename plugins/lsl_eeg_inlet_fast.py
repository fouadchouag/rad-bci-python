...
class LSL_EEG_Inlet_Fast(BasePlugin):
    ...
    help = help = { 'gotchas': [ 'Only the latest chunk is kept (DropOldQueue); intermediate data between ticks is discarded.',
  ...
  'inputs': { 'config_in': 'dict — merged configuration block (keys: emit_ms, chunk_ms, stream_name, autoconnect)',
  ...
  'parameters': [ { 'default': 100,
                     ...
                     'type': 'int'},
                   { 'default': 20,
                     'desc': 'Duration of each LSL pull in milliseconds',
                     'name': 'chunk_ms',
                     'type': 'int'},
                   { 'default': False,
                     ...
                     'name': 'autoconnect',
                     'type': 'bool'}],
  'summary': 'Inlet EEG non-bloquant : thread lecteur + DropOldQueue + QTimer emit.',
  'usage': 'Use the UI to select and connect to an EEG stream, or send a config dict via config_in/lsl_eeg_conf inputs to autoconnect programmatically.'}
