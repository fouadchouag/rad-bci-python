...
class MNEEpochsLite(BasePlugin):
    help = help = {
        'summary': 'Minimal, robust epoching with automatic event detection (annotations → STIM → fixed-length → manual fallback).',
        'usage': 'Connect a Raw object. The plugin auto-detects events and creates fixed-duration epochs. Route output to feature or display nodes.',
        'inputs': {
        ...
        'parameters': [
            {'name': 'use_annotations', 'type': 'bool', 'default': True, 'desc': 'Try to extract events from Raw annotations before falling back'},
            {'name': 'epoch_len_s', 'type': 'float', 'default': 1.0, 'desc': 'Duration of each epoch in seconds (minimum 0.05)'},
            {'name': 'step_s', 'type': 'float', 'default': 1.0, 'desc': 'Step between epoch onsets in seconds (minimum 0.05)'},
        ],
        'gotchas': [
