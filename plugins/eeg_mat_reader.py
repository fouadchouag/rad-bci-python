...
class EEGMatReader(BasePlugin):
    help = { 'gotchas': ['Requires scipy (v7.2 MAT) or h5py (v7.3 HDF5-MAT).',
      ...
      'inputs': {},
      'outputs': { 'segment': '2D float array [ch x samples] — streamed EEG chunk',
                   'ch_names': 'list[str] — channel names',
                   'sfreq': 'float — sampling rate (Hz)',
                   'info': 'dict — file metadata (path, style, mode, n_channels)'},
      'parameters': [ { 'default': '',
                        'desc': 'MAT file path (set via Open button)',
                        'name': 'filepath',
                        'type': 'path'},
                      { 'default': 1.0,
                        'desc': 'Chunk duration for Continuous mode',
                        'name': 'chunk_s',
                        'type': 'float',
                        'unit': 's'},
                      { 'default': 0.0,
                        'desc': 'Overlap between consecutive chunks',
                        'name': 'overlap_s',
                        'type': 'float',
                        'unit': 's'},
                      { 'default': 'Trials',
                        ...
                        'name': 'mode',
                        'type': 'str',
                        'enum': ['Trials', 'Continuous']},
                      { 'default': False,
                        'desc': 'Loop playback',
                        'name': 'loop',
                        'type': 'bool'},
                      { 'default': True,
                        'desc': 'Start playing immediately after load',
                        'name': 'auto_play',
                        'type': 'bool'}],
      'summary': 'Read .mat EEG files (BBCI/BCI Competition/generic) and stream segments.',
      'usage': 'Place at pipeline start; connect `segment` to streaming processors. '
               'Use "Infos fichier" to inspect loaded data.'}
