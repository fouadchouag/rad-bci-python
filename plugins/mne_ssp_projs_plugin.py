...
class MNEComputeSSPProjs(BasePlugin):
    help = {
        'summary': 'Compute SSP projectors for EOG/ECG artifact rejection on MNE Raw objects.',
        'inputs': {
        ...
        'parameters': [
            {'name': 'n_eog', 'type': 'int', 'default': 2, 'desc': 'Number of EOG SSP projectors to compute.'},
            {'name': 'l_freq_eog', 'type': 'float', 'default': 1.0, 'desc': 'Low cutoff frequency (Hz) for EOG filter.'},
            {'name': 'h_freq_eog', 'type': 'float', 'default': 10.0, 'desc': 'High cutoff frequency (Hz) for EOG filter.'},
            {'name': 'ch_name_eog', 'type': 'str', 'default': '', 'desc': 'EOG channel name (empty = auto-detect).'},
            {'name': 'n_ecg', 'type': 'int', 'default': 2, 'desc': 'Number of ECG SSP projectors to compute.'},
            {'name': 'l_freq_ecg', 'type': 'float', 'default': 8.0, 'desc': 'Low cutoff frequency (Hz) for ECG filter.'},
            {'name': 'h_freq_ecg', 'type': 'float', 'default': 20.0, 'desc': 'High cutoff frequency (Hz) for ECG filter.'},
            {'name': 'ch_name_ecg', 'type': 'str', 'default': '', 'desc': 'ECG channel name (empty = auto-detect).'},
        ],
        'gotchas': [
        ...
        'usage': 'Connect an MNE Raw node upstream. Use the UI buttons to compute EOG/ECG projectors, or create virtual reference channels first if no dedicated EOG/ECG channel exists.',
    }
