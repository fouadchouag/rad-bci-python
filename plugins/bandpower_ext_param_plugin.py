...
class BandpowerExt(BasePlugin):
    help = {
        'summary': 'Extract per-channel band power features using a built-in Welch-like estimator (no SciPy required).',
        'inputs': {
        ...
        'parameters': [
            {'name': 'preset', 'type': 'str', 'default': 'MI (alpha,beta)',
             'desc': 'Band preset: "MI (alpha,beta)" → [alpha, beta] or "Full (delta,theta,alpha,beta)" → [delta, theta, alpha, beta]. Selectable from UI combo box.'},
            {'name': 'relative', 'type': 'bool', 'default': True,
             'desc': 'If True, band powers are normalized by total power in the 1–40 Hz range (relative power). Toggle from UI checkbox.'},
            {'name': 'nperseg', 'type': 'int', 'default': 256,
             'desc': 'Welch segment length in samples (32–4096). Clamped to data length if shorter. Configurable from UI.'},
        ],
        'gotchas': [
        ...
        'usage': 'Connect an EEG segment and optionally sfreq/ch_names. Select preset and relative mode from the collapsible parameters section. Outputs per-channel band power dict.',
    }
