...
class WebFeedbackClient(BasePlugin):
    help = {
        'summary': 'HTTP client that POSTs feedback (label, confidence, payload) to a ServerHttpLauncher /feedback endpoint.',
        'inputs': {
        ...
        'parameters': [
            {'name': 'host', 'type': 'str', 'default': '127.0.0.1', 'desc': 'Target server host.'},
            {'name': 'port', 'type': 'int', 'default': 8000, 'desc': 'Target server port.'},
        ],
        'gotchas': [
        ...
        'usage': 'Connect label (and optionally confidence/payload) from a classifier upstream. Set the host and port of a running ServerHttpLauncher. Each execute() sends a POST /feedback with the current values.',
    }
    """
    Entrées:
