...
class LSL_EEG_Inlet(BasePlugin):
    help = help = { 'gotchas': [ 'Verify channels and sampling rate after connecting.',
  ...
  'parameters': [ { 'default': 20,
                     ...
                     'type': 'int'},
                   { 'default': 50,
                     'desc': 'Interval between emit ticks in milliseconds (QTimer)',
                     'name': 'emit_ms',
                     'type': 'int'},
                   { 'default': 5.0,
                     'desc': 'Maximum buffer duration in seconds before dropping old data',
                     'name': 'buffer_max_s',
                     'type': 'float'}],
  'summary': 'Inlet LSL générique pour flux EEG (float32, multi-canaux).',
  'usage': 'Use the UI to refresh/connect to an EEG LSL stream; outputs are emitted periodically via QTimer.'}
