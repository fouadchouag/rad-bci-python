...
class AcquisitionManager(BasePlugin):
    help = help = { 'gotchas': ['Native hardware driver is disabled (placeholder only).',
               ...
               'Emulator generates a 10 Hz + 20 Hz sine signal with configurable noise.'],
                   'inputs': {},
                   'outputs': {'ch_names': 'List[str] — channel labels from the selected source',
                   ...
                   'parameters': [ { 'default': 'LSL',
                                      'desc': 'Data source driver: LSL, Emulator, or Native (disabled)',
                                      'name': 'source',
                                      'type': 'str'},
                                    { 'default': 0.0,
                                      'desc': 'Segment length in seconds (0 = auto power-of-2)',
                                      'name': 'seg_len_s',
                                      'type': 'float'},
                                    { 'default': 0.5,
                                      'desc': 'Hop ratio (0..1) as fraction of segment length',
                                      'name': 'hop_ratio',
                                      'type': 'float'},
                                    { 'default': 0.0,
                                      'desc': 'Hop in seconds (used only when hop_ratio=0)',
                                      'name': 'hop_s',
                                      'type': 'float'},
                                    { 'default': True,
                                      'desc': 'Apply Hann edge smoothing to segments',
                                      'name': 'smoothing',
                                      'type': 'bool'}],
                   'summary': 'AcquisitionManager (léger) — LSL | Emulator | Native (disabled)',
                   'usage': 'Select a source in the UI, scan and pick an LSL stream, then press Start. Segments are emitted automatically via the segmentation engine.'}
