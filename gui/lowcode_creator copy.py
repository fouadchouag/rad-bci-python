...
class IOTable(QTableWidget):
    def __init__(self, title: str, parent=None):
        ...
        hdr.setMinimumSectionSize(160)  # ➜ type bien lisible
...
class ParamTable(QTableWidget):
    ...
    def __init__(self, parent=None):
        ...
        hdr.setSectionResizeMode(2, QHeaderView.ResizeToContents)   # default
        hdr.setSectionResizeMode(3, QHeaderView.ResizeToContents)   # min
        hdr.setSectionResizeMode(4, QHeaderView.ResizeToContents)   # max
        hdr.setSectionResizeMode(5, QHeaderView.ResizeToContents)   # step
        hdr.setSectionResizeMode(6, QHeaderView.ResizeToContents)   # unit
        hdr.setSectionResizeMode(7, QHeaderView.Stretch)            # choices
...
class LowCodeCreator(QWidget):
    ...
    def _apply_filter_preset(self):
        # Paramètres UI classiques d’un band-pass + notch
    ...
    def _polyglot_business_skeleton(self, name: str, lang: str, ins: List[IOPort], outs: List[IOPort], params: List[ParamSpec]) -> str:
        # construit les commentaires d’I/O
        def _in_fields():
            f = []
            for p in ins:
...
fn main() {{
    ...
    if args.iter().any(|a| a == "--stdio") {{
        ...
        for line in stdin.lock().lines() {{
            ...
            // TODO: lire les champs ici, appliquer le traitement DSP, etc.
            // let _fs = data["sfreq"].as_f64().unwrap_or(0.0);
            // let _raw = &data["raw"]; // [C][N] si envoyé en JSON
...
"""
    ...
    def _add_to_palette(self):
        ...
        wrapper_code = self._generate_typed_wrapper(
            ...
            language=lang_disp, params=params
        )
    ...
    def _generate_typed_wrapper(self, display_name: str, class_name: str, slug: str,
                                ...
                                language: str, params: List[ParamSpec]) -> str:
        """
        Wrapper BasePlugin:
          ...
          - Ajoute les paramètres UI dans le payload JSON (HP/LP/…)
          - Garde-fous Nyquist si 'sfreq' présent
          - 🔁 Recalcule automatiquement (debounce 300 ms) quand un paramètre change
          - ⚡ Prévisualisation rapide (fenêtre & décimation) vers binaire C/Rust
          - 🧠 Cache .tolist() pour RAW inchangé
          - 🚀 **Nouveau**: appel persistant du binaire via --stdio (stdin/stdout), fallback fichiers intact
          - 🧭 **Mode d'exécution** sélectionnable dans l'UI: Auto / STDIO / Subprocess
        """
        ...
        # ParamDefs → Python literal (+ fast-preview params)
        def _param_defs_literal(ps: List[ParamSpec]) -> str:
            ...
            for p in ps:
                ...
                arr.append(d)
            # Ajout des contrôles de prévisualisation
        ...
        in_ser_lines = [
            ...
            "__raw_arr_tmp = None",  # buffer pour optimisation RAW
        ]
            ...
            if t == "ndarray_2d":
                if n == "raw":
                    in_ser_lines += [
                        ...
                        f"    __raw_arr_tmp = _to_ndarray2d(val)   # (C,N)",
                        "else:",
                ...
                if n == "raw":
                    in_ser_lines += [
                        f"val = kwargs.get('{n}', None)",
                        f"if val is not None:",
                        f"    __raw_arr_tmp = _to_ndarray2d(val)   # (C,N) via Raw.get_data()",
                        f"    try:",
                        f"        fs = float(getattr(val, 'info', {{}}).get('sfreq', 0.0) or 0.0)",
                        f"        if fs>0: payload['sfreq'] = fs",
                    ...
                    in_ser_lines += [
                        f"val = kwargs.get('{n}', None)",
                        f"if val is not None:",
                        f"    _arr = _to_ndarray2d(val)   # (C,N) via Raw.get_data()",
                        f"    payload['{n}'] = _arr.tolist()",
                        f"    try:",
                        f"        fs = float(getattr(val, 'info', {{}}).get('sfreq', 0.0) or 0.0)",
                        f"        if fs>0: payload['{n}_fs'] = fs",
                        f"    except Exception: pass",
                        "else:",
                        f"    {'missing.append(%r)' % n if is_required else 'pass'}",
                    ]
        ...
        # Fin de sérialisation: RAW -> preview & cache + fail si inputs requis manquants
        in_ser_lines += [
            "# 🔒 si une entrée requise manque, on ne lance pas le binaire",
            "if missing:",
            "    return {}",
            "",
            "# ⚡ Prévisualisation + cache tolist() pour RAW",
            "try:",
            "    import numpy as _np",
            ...
            "            if _dec > 1:",
            "                arr = arr[:, ::_dec]",
            "                payload['sfreq'] = float(fs / _dec)",
            "        # cache list() par id + shape + fs",
        ...
        # ---- Template à jetons (aucune interpolation par .format / f-string) ----
        template = r'''
...
# ⚡ Prévisualisation (fenêtre + décimation) envoyée au binaire C/Rust
# 🧠 Cache tolist() pour RAW inchangé
# 🚀 **Persistance**: tente d'utiliser --stdio (stdin/stdout). Fallback fichiers si non supporté.
# 🧭 **Mode d'exécution**: Auto (stdio→fichiers), STDIO (persistant), Subprocess (fichiers)
...
class __CLASS_NAME__Plugin(BasePlugin):
    ...
    def setup(self):
        ...
        self._proc = None
        self._exec_mode = "auto"  # "auto" | "stdio" | "subprocess"
        self._stdio_cmd = None

    # ---------- worker stdio ----------
    def _stdio_command(self):
        ext = os.path.splitext(self.executable)[1].lower()
        if ext in (".exe", ""):
            return [self.executable, "--stdio"]
        if ext == ".js":
            return ["node", self.executable, "--stdio"]
        if ext == ".py":
            return ["python", self.executable, "--stdio"]
        if ext == ".r":
            return ["Rscript", self.executable, "--stdio"]
        if ext == ".jl":
            return ["julia", self.executable, "--stdio"]
        if ext == ".sh":
            return ["bash", self.executable, "--stdio"]
        if ext == ".m":
            # Pas vraiment du stdio, mais on tente: l'app devra lire stdin par elle-même
            return ["octave", "--quiet", "--eval", "run('%s')" % self.executable]
        return [self.executable, "--stdio"]
    ...
    def _ensure_worker(self):
        if self._proc and self._proc.poll() is None:
            return
        try:
            cmd = self._stdio_cmd or self._stdio_command()
            self._proc = subprocess.Popen(
                cmd,
                stdin=subprocess.PIPE,
                stdout=subprocess.PIPE,
                stderr=subprocess.PIPE,
                text=True,            # lignes JSON
    ...
    def build_widget(self):
        try:
            from PyQt5.QtWidgets import (
                ...
                QCheckBox, QComboBox, QLineEdit, QToolButton, QSizePolicy, QLabel, QHBoxLayout
            )
        ...
        # --- barre mode d'exécution ---
        exec_bar = QWidget(root)
        hb = QHBoxLayout(exec_bar); hb.setContentsMargins(0,0,0,0); hb.setSpacing(6)
        hb.addWidget(QLabel("Mode d'exécution:"))
        exec_cb = QComboBox(exec_bar)
        exec_cb.addItems(["Auto (stdio→fichiers)", "STDIO (persistant)", "Subprocess (fichiers)"])
        hb.addWidget(exec_cb, 1)
        hb.addStretch(1)
        vbox.addWidget(exec_bar)

        def _on_exec_mode_changed():
            txt = exec_cb.currentText()
            self._exec_mode = "auto" if txt.startswith("Auto") else ("stdio" if txt.startswith("STDIO") else "subprocess")
        try:
            exec_cb.currentIndexChanged.connect(_on_exec_mode_changed)
        except Exception:
            pass
        _on_exec_mode_changed()

        # --- en-tête repliable paramètres ---
        header = QToolButton(root)
        header.setText("Paramètres")
        header.setCheckable(True)
        header.setChecked(True)                    # ouvert par défaut
        ...
        header.toggled.connect(_update_arrow)
        _update_arrow(True)
        ...
        for d in (PARAM_DEFS or []):
            ...
            if ptype in ("float","sfreq"):
                ...
                sb.setSingleStep(st)
                dv = _f(d.get("default",""), 0.0) or 0.0
                sb.setValue(dv)
                widget = sb
            elif ptype == "int":
            ...
            # 🔁 relancer calcul quand le param change (debounce)
            try:
    ...
    def export_config(self):
        ...
        return {"params": params, "exec_mode": getattr(self, "_exec_mode", "auto")}
        ...
        try:
            ...
            for k,(ptype, widget, d) in (self._param_widgets or {}).items():
                ...
                try:
                    ...
                    pass
            self._exec_mode = (cfg or {}).get("exec_mode", "auto")
        except Exception:
            pass
    ...
    # ---------- Coercion sorties ----------
    def _coerce_outputs(self, result, payload, kwargs):
        """
        Normalise le dict 'result' venant du moteur (in-process/stdio/subprocess)
        vers les pins de sortie: matrices [[C][N]] -> ndarray (N,C), etc.
        Essaie de renvoyer mne.RawArray pour 'raw' si sfreq connu.
        """
        import numpy as _np
        out_dict = {}
        ...
        for k, v in result.items():
            try:
                if isinstance(v, (list, tuple)) and v and isinstance(v[0], (list, tuple)):
                    _C_N = _np.asarray(v, dtype=_np.float32)
                    out_dict[k] = _C_N.T
                    if k == "raw":
                        try:
                            import mne
                            _fs = float(
                                (kwargs or {}).get("sfreq", 0.0)
                                or (payload or {}).get("sfreq", 0.0)
                                or 0.0
                            )
                            if _fs > 0:
                                _C = _C_N.shape[0]
    ...
    def execute(self, **kwargs):
        # 0) lire paramètres UI + éventuellement ajuster bornes Nyquist
        params = self._gather_params_safe()

        # 1) Construire payload (attend toutes les entrées requises)
        __IN_SER__

        # 1.b) Fusionner les paramètres UI dans le payload
        try:
            payload.update(params or {})
        except Exception:
            pass

        # 1.c) Garde-fous typiques (Nyquist, HP<LP) si sfreq présent
        try:
            fs = float(kwargs.get("sfreq", 0.0) or payload.get("sfreq", 0.0) or 0.0)
            if fs > 0:
        ...
        # 2) Appel binaire: en fonction du mode sélectionné
        mode = getattr(self, "_exec_mode", "auto")
        result = None

        if mode in ("auto", "stdio"):
            try:
                # timeout un peu plus large si gros paquets
                result = self._rpc_call(payload, timeout_s=3.0)
            except Exception:
                if mode == "stdio":
                    raise
                # auto -> retry fresh puis fallback fichiers
                try:
                    self._kill_worker()
                    result = self._rpc_call(payload, timeout_s=3.0)
                except Exception:
        ...
        # 3) Mapper le JSON de sortie vers les pins
        out_dict = self._coerce_outputs(result, payload, kwargs)
        return out_dict

    def on_remove(self):
        # fermer proprement le worker persistant si présent
        try:
            self._kill_worker()
        except Exception:
            pass
'''
        # Remplacements sûrs (jetons uniques)
