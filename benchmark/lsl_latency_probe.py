...
# Detection threshold: burst peaks at ~100 (unfiltered) or ~80-100 (filtered).
# Background noise+signal on ch0 stays near 0. A threshold of 50 separates
# them cleanly with a wide safety margin.
PULSE_THRESHOLD = 50.0

# Refractory period: after detecting a burst, ignore crossings for this long.
# The burst itself lasts ~50 ms and oscillates (20 Hz), so it crosses the
# threshold up-down multiple times. We want ONE detection per burst.
# 200 ms > 50 ms burst duration + margin for filter ringing.
REFRACTORY_S = 0.2
...
def detect_pulse_events(chunk, timestamps, state):
    """
    Find burst-onset events in a chunk.

    A burst is a 50-ms 20-Hz sinusoid; it oscillates and crosses the
    threshold several times. We detect the FIRST crossing (rising edge)
    of a new burst and then ignore further crossings for REFRACTORY_S
    seconds.

    Returns list of (local_receive_time, pulse_index) for each detected
    burst onset. The 'state' dict tracks 'last_detect_t' and 'next_pulse_idx'
    across calls.
    """
    events = []
    if chunk is None:
    ...
    for i, sample in enumerate(chunk):
        ...
        if v > PULSE_THRESHOLD:
            # Only fire if we are past the refractory window
            if now - state["last_detect_t"] > REFRACTORY_S:
                events.append((now, state["next_pulse_idx"]))
                state["next_pulse_idx"] += 1
                state["last_detect_t"] = now
            # else: still inside refractory, ignore
    return events
...
def run_probe(duration_s: float, output_csv: str, require_output: bool = True):
    ...
    src_state = {"last_detect_t": -1e9, "next_pulse_idx": 0}
    out_state = {"last_detect_t": -1e9, "next_pulse_idx": 0}
